...
def switch_keypad_layout(driver: WebDriver, target_layout: str) -> bool:
    try:
        ...
        # We can find the option button via XPath to be completely safe
        ...
        if topic_el:
            ...
            if matching_card:
                ...
                if wait_for_loading(driver):
                    ...
                    # ── TEST QUESTION 5 (expected: \sqrt{34}) ──
                    print("\n--- TESTING Q5 ---")
                    ...
                    if visible_inputs:
                        print("Focusing on Q5 math input...")
                        driver.execute_script("arguments[0].click();", visible_inputs[0])
                        time.sleep(1.0)
                        
                        # Enter answer
                        enter_math_answer(driver, "\\sqrt{34}")
                        time.sleep(2.0)
                        
                        # Save screenshot
                        os.makedirs("screenshots", exist_ok=True)
                        driver.save_screenshot("screenshots/test_keypad_q5_after_xpath.png")
                        print("Saved screenshots/test_keypad_q5_after_xpath.png")
                        
                        # Let's inspect the mathquill latex value
                        latex_val = driver.execute_script(
                            "try { "
                            "  if (typeof MQ !== 'undefined') {"
                            "    return MQ.MathField(arguments[0]).latex();"
                            "  }"
                            "  let $el = jQuery(arguments[0]);"
                            "  let mq = $el.data('MathQuill');"
                            "  if (mq) return mq.latex();"
                            "} catch(e) {}"
                            "return null;", visible_inputs[0]
                        )
                        print(f"MathQuill LaTeX in field: '{latex_val}'")
                    else:
                        print("No visible math input found for Q5")
